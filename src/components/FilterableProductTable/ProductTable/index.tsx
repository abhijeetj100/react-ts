...
export default function ProductTable({ProductData}: {ProductData: IProduct[]}){
    ...
    uniqCategories.forEach((category) => {
        productsByCategory.set(category, ProductData.filter((product) => product.category === category));
    });

    let productCaregoriesComponents: Map<string, IProductCategoryRow> = new Map<string, IProductCategoryRow>();
    ...
    productsByCategory.forEach((value, key) => {
        productCaregoriesComponents.set(key, {
            ...
            productComponents: value.map((product) => <ProductRow product={product} />)
        });
        tableRows.push(<ProductCategoryRow category={key} />);
        tableRows.push(...value.map((product) => <ProductRow product={product} />));
    })
